...
int udhcpc_main(int argc UNUSED_PARAM, char **argv)
{
	...
	if (udhcp_read_interface(client_config.interface,
			&client_config.ifindex,
			NULL,
			client_config.client_mac, &ifflags)
	) {
		return 1;
	}
		...
		if (retval == 0) {
			...
			case INIT_SELECTING:
				if (!discover_retries || packet_num < discover_retries) {
					if (ifflags & IFF_LOWER_UP != IFF_LOWER_UP) {
						wait_for_carrier(client_config.interface);
					}
					if (packet_num == 0)
						xid = random_xid();
					/* broadcast */
...
int read_msg(int fd)
{
	...
	for (nh = (struct nlmsghdr *) buf; NLMSG_OK (nh, len);
		...
		if (ifimsg->ifi_change & IFF_LOWER_UP == IFF_LOWER_UP)
			return 0;
	}
}

int wait_for_carrier(const char *interface) {
	struct sockaddr_nl sa;
	int fd;
